...
def move_electrode_to(electrode, segment_or_segment_name):
    if type(segment_or_segment_name)==str:
        segment = eval('h.' + segment_or_segment_name)
    else:
        segment = segment_or_segment_name
    h.push_section(str(segment.sec))
    electrode.loc(segment.x)
    h.pop_section()


def locate_electrode(electrode):
    x = electrode.get_loc()
    sec = h.cas()
    secname = sec.name()
    h.pop_section()
    return '{}({})'.format(secname, x)
